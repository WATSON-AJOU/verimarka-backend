...
class ContentRegisterViewTests(TestCase):
    ...
    def test_register_image_enqueues_async_job(self, mocked_delay, *_mocks):
        ...
        response = self.client.post(reverse("content_register"), {"file": upload}, format="multipart")
        ...
        self.assertTrue(AIJob.objects.filter(content=content, job_type="register").exists())

    def test_register_rejects_unsupported_file(self):
        upload = SimpleUploadedFile("sample.txt", b"plain text", content_type="text/plain")
    ...
    def test_register_accepts_document_file(self, mocked_delay, *_mocks):
        ...
        upload = SimpleUploadedFile("sample.pdf", b"%PDF-1.4", content_type="application/pdf")
    ...
    def test_register_rejects_mime_and_extension_mismatch(self):
        upload = SimpleUploadedFile("sample.png", b"\xff\xd8\xff\xe0fakejpegcontent", content_type="image/jpeg")
    ...
    def test_register_blocks_duplicate_source_after_success(self, mocked_delay, *_mocks):
        mocked_delay.return_value.id = "celery-task-1"
        first_upload = SimpleUploadedFile("sample.png", b"\x89PNG\r\n\x1a\nsamecontent", content_type="image/png")
        first_response = self.client.post(reverse("content_register"), {"file": first_upload}, format="multipart")
        ...
        second_upload = SimpleUploadedFile("sample.png", b"\x89PNG\r\n\x1a\nsamecontent", content_type="image/png")
        second_response = self.client.post(reverse("content_register"), {"file": second_upload}, format="multipart")

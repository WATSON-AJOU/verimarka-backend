...
from wallets.models import WalletConnectionChallenge, WalletLink
...
class WalletLinkView(APIView):
    ...
    def delete(self, request):
        ...
        if wallet_link is not None:
            logger.info("wallet.unlink.success user_id=%s address=%s", request.user.id, wallet_link.address)
            wallet_link.delete()
...
class WalletSummaryView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            nft_count = int(blockchain.contract.functions.balanceOf(Web3.to_checksum_address(wallet_link.address)).call())
...
class WalletConnectChallengeView(APIView):
    ...
    def post(self, request):
        ...
        WalletConnectionChallenge.objects.filter(user=request.user, used_at__isnull=True).update(used_at=timezone.now())
...
class WalletConnectVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if challenge is None:
            return Response({"message": "지갑 연결 요청이 만료되었습니다. 다시 시도해주세요."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            recovered_address = Account.recover_message(encode_defunct(text=challenge.message), signature=signature)
            recovered_address = _normalize_address(recovered_address)
        except Exception:
            return Response({"message": "서명 검증에 실패했습니다. 다시 시도해주세요."}, status=status.HTTP_400_BAD_REQUEST)

        if recovered_address != address:
            return Response({"message": "현재 선택한 지갑과 서명한 지갑 주소가 일치하지 않습니다."}, status=status.HTTP_400_BAD_REQUEST)

        conflict_link = WalletLink.objects.filter(address=address).exclude(user=request.user).first()
        if conflict_link is not None:
            return Response({"message": "이미 다른 계정에 연결된 지갑 주소입니다."}, status=status.HTTP_409_CONFLICT)

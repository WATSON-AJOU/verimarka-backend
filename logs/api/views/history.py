...
class AnalysisHistoryView(APIView):
    ...
    def get(self, request):
        ...
        merged = sorted(content_items + verify_items, key=lambda item: item["sort_key"], reverse=True)
        return Response(
            [
                {
                    "id": item["id"],
    ...
    def _serialize_content(self, content: Content, request):
        ...
        original_preview_url, watermark_preview_url = content_image_urls(content, request)
        ...
        if is_rejected_review_vote_result(content):
            ...
            item = {
                ...
                "comparison_public_id": comparison_public_id,
                "comparison_label": comparison_label,
                "download_url": reverse("content_watermark_download", kwargs={"public_id": content.public_id}) if watermark_applied else None,
                "blockchain": blockchain,
                "sort_key": content.created_at,
            }
            ...
            extra = f"찬성 {vote.get('upvotes', 0)} · 반대 {vote.get('downvotes', 0)}" if vote else "커뮤니티 검증 필요"
            comparison_preview_url, comparison_file_name, comparison_public_id, _ = resolve_history_candidate(content, request)
            comparison_label = "유사 후보"
        else:
            summary = "동일한 저작물 재업로드로 차단" if is_exact_duplicate_block(content) else "등록 차단"
    ...
    def _serialize_verification(self, item: VerificationHistoryLog):
        return {
            ...
            "summary": item.summary or ("워터마크 검증 성공" if item.outcome == "verified" else "검증 실패"),
            ...
            "extra": (item.candidate or {}).get("summary") or ((item.blockchain or {}).get("verification_link") or "-"),
            ...
            "comparison_label": "유사 후보" if (item.candidate or {}).get("preview_url") else "",
            "download_url": None,
...
class PublicRecentActivityView(APIView):
    def get(self, request):
        payload = []
        for content in Content.objects.exclude(status="pending").order_by("-updated_at")[:12]:
            blockchain = content.blockchain or {}
            payload.append(
                {
                    "id": str(content.public_id),
